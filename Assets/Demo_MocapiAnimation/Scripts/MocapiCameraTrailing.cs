...
public class MocapiCameraTrailing : MonoBehaviour
{
    ...
    void PositionChange()
    {
        ...
        if (Input.GetKey(KeyCode.Keypad8) || (Input.GetAxis("HatVertical") < -0.1f)) //from behind
        {
            standardPos = CamPosBehind.transform;
        }
        else if (Input.GetKey(KeyCode.Keypad2) || (Input.GetAxis("HatVertical") > 0.1f)) //from front
        {
            standardPos = CamPosFront.transform;
        }
        else if (Input.GetKey(KeyCode.Keypad4) || (Input.GetAxis("HatHorizontal") > 0.1f)) //from left
        {
            standardPos = CamPosLeft.transform;
        }
        else if (Input.GetKey(KeyCode.Keypad6) || (Input.GetAxis("HatHorizontal") < -0.1f)) //from right
        {
